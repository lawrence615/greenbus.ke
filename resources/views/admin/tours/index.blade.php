                        ...
                        <td class="px-6 py-4 text-right">
                            <div class="flex items-center justify-end gap-2">
                                ...
                                <form method="POST" action="{{ route('console.tours.toggle-status', $tour) }}" class="inline">
                                    @csrf
                                    @method('PATCH')
                                    <button type="submit" class="{{ $tour->status === 'published' ? 'text-yellow-600 hover:text-yellow-700' : 'text-blue-600 hover:text-blue-700' }}" title="{{ $tour->status === 'published' ? 'Unpublish' : 'Publish' }}">
